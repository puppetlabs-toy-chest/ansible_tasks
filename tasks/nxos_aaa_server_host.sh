...
if [ ! -z "${PT_auth_port+x}" ];
  then params="${params} auth_port=\"${PT_auth_port}\""
fi

if [ ! -z "${PT_host_timeout+x}" ];
  then params="${params} host_timeout=\"${PT_host_timeout}\""
fi

if [ ! -z "${PT_tacacs_port+x}" ];
  then params="${params} tacacs_port=\"${PT_tacacs_port}\""
fi
...
if [ ! -z "${PT_acct_port+x}" ];
  then params="${params} acct_port=\"${PT_acct_port}\""
fi
...
ansible localhost -m nxos_aaa_server_host --args "${params}" --one-line | cut -f2- -d">"