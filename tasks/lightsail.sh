...
if [ ! -z "${PT_blueprint_id+x}" ];
  then params="${params} blueprint_id=\"${PT_blueprint_id}\""
fi
...
if [ ! -z "${PT_key_pair_name+x}" ];
  then params="${params} key_pair_name=\"${PT_key_pair_name}\""
fi
...
if [ ! -z "${PT_bundle_id+x}" ];
  then params="${params} bundle_id=\"${PT_bundle_id}\""
fi
...
ansible localhost -m lightsail --args "${params}" --one-line | cut -f2- -d">"