...
if [ ! -z "${PT_trunk_allowed_vlans+x}" ];
  then params="${params} trunk_allowed_vlans=\"${PT_trunk_allowed_vlans}\""
fi
...
ansible localhost -m nxos_switchport --args "${params}" --one-line | cut -f2- -d">"