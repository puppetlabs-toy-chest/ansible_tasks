...
if [ ! -z "${PT_enable_availability_zone+x}" ];
  then params="${params} enable_availability_zone=\"${PT_enable_availability_zone}\""
fi

if [ ! -z "${PT_ec2_elbs+x}" ];
  then params="${params} ec2_elbs=\"${PT_ec2_elbs}\""
fi

ansible localhost -m ec2_elb --args "${params}" --one-line | cut -f2- -d">"