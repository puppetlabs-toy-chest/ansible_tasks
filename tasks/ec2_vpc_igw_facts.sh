...
if [ ! -z "${PT_internet_gateway_ids+x}" ];
  then params="${params} internet_gateway_ids=\"${PT_internet_gateway_ids}\""
fi
...
ansible localhost -m ec2_vpc_igw_facts --args "${params}" --one-line | cut -f2- -d">"