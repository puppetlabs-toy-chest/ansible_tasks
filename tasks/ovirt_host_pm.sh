...
if [ ! -z "${PT_encrypt_options+x}" ];
  then params="${params} encrypt_options=\"${PT_encrypt_options}\""
fi
...
if [ ! -z "${PT_order+x}" ];
  then params="${params} order=\"${PT_order}\""
fi

ansible localhost -m ovirt_host_pm --args "${params}" --one-line | cut -f2- -d">"