...
if [ ! -z "${PT_peering_id+x}" ];
  then params="${params} peering_id=\"${PT_peering_id}\""
fi
...
if [ ! -z "${PT_peer_vpc_id+x}" ];
  then params="${params} peer_vpc_id=\"${PT_peer_vpc_id}\""
fi

if [ ! -z "${PT_peer_owner_id+x}" ];
  then params="${params} peer_owner_id=\"${PT_peer_owner_id}\""
fi
...
ansible localhost -m ec2_vpc_peer --args "${params}" --one-line | cut -f2- -d">"