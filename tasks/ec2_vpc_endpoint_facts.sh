...
if [ ! -z "${PT_vpc_endpoint_ids+x}" ];
  then params="${params} vpc_endpoint_ids=\"${PT_vpc_endpoint_ids}\""
fi
...
ansible localhost -m ec2_vpc_endpoint_facts --args "${params}" --one-line | cut -f2- -d">"