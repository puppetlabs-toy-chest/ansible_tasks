...
if [ ! -z "${PT_anycast_gateway_mac+x}" ];
  then params="${params} anycast_gateway_mac=\"${PT_anycast_gateway_mac}\""
fi
...
ansible localhost -m nxos_overlay_global --args "${params}" --one-line | cut -f2- -d">"