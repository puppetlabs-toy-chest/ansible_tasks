...
if [ ! -z "${PT_task+x}" ];
  then params="${params} task=\"${PT_task}\""
fi
...
if [ ! -z "${PT_started_by+x}" ];
  then params="${params} started_by=\"${PT_started_by}\""
fi
...
if [ ! -z "${PT_container_instances+x}" ];
  then params="${params} container_instances=\"${PT_container_instances}\""
fi
...
if [ ! -z "${PT_overrides+x}" ];
  then params="${params} overrides=\"${PT_overrides}\""
fi

ansible localhost -m ecs_task --args "${params}" --one-line | cut -f2- -d">"