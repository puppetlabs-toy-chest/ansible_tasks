...
if [ ! -z "${PT_msg_time+x}" ];
  then params="${params} msg_time=\"${PT_msg_time}\""
fi
...
if [ ! -z "${PT_aggressive+x}" ];
  then params="${params} aggressive=\"${PT_aggressive}\""
fi
...
ansible localhost -m nxos_udld --args "${params}" --one-line | cut -f2- -d">"