...
if [ ! -z "${PT_port_range_max+x}" ];
  then params="${params} port_range_max=\"${PT_port_range_max}\""
fi
...
if [ ! -z "${PT_remote_ip_prefix+x}" ];
  then params="${params} remote_ip_prefix=\"${PT_remote_ip_prefix}\""
fi

if [ ! -z "${PT_remote_group+x}" ];
  then params="${params} remote_group=\"${PT_remote_group}\""
fi
...
if [ ! -z "${PT_port_range_min+x}" ];
  then params="${params} port_range_min=\"${PT_port_range_min}\""
fi
...
if [ ! -z "${PT_ethertype+x}" ];
  then params="${params} ethertype=\"${PT_ethertype}\""
fi
...
ansible localhost -m os_security_group_rule --args "${params}" --one-line | cut -f2- -d">"