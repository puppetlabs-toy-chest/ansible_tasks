...
if [ ! -z "${PT_vm_per_user+x}" ];
  then params="${params} vm_per_user=\"${PT_vm_per_user}\""
fi
...
if [ ! -z "${PT_prestarted+x}" ];
  then params="${params} prestarted=\"${PT_prestarted}\""
fi
...
if [ ! -z "${PT_vm_count+x}" ];
  then params="${params} vm_count=\"${PT_vm_count}\""
fi
...
ansible localhost -m ovirt_vmpools --args "${params}" --one-line | cut -f2- -d">"