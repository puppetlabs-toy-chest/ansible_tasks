...
if [ ! -z "${PT_task_role_arn+x}" ];
  then params="${params} task_role_arn=\"${PT_task_role_arn}\""
fi
...
ansible localhost -m ecs_taskdefinition --args "${params}" --one-line | cut -f2- -d">"