...
if [ ! -z "${PT_publisher+x}" ];
  then params="${params} publisher=\"${PT_publisher}\""
fi

if [ ! -z "${PT_offer+x}" ];
  then params="${params} offer=\"${PT_offer}\""
fi
...
ansible localhost -m azure_rm_virtualmachineimage_facts --args "${params}" --one-line | cut -f2- -d">"