...
if [ ! -z "${PT_peer_link+x}" ];
  then params="${params} peer_link=\"${PT_peer_link}\""
fi

if [ ! -z "${PT_portchannel+x}" ];
  then params="${params} portchannel=\"${PT_portchannel}\""
fi
...
ansible localhost -m nxos_vpc_interface --args "${params}" --one-line | cut -f2- -d">"