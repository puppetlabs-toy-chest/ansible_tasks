...
if [ ! -z "${PT_system_mtu+x}" ];
  then params="${params} system_mtu=\"${PT_system_mtu}\""
fi
...
if [ ! -z "${PT_domain_lookup+x}" ];
  then params="${params} domain_lookup=\"${PT_domain_lookup}\""
fi
...
ansible localhost -m nxos_system --args "${params}" --one-line | cut -f2- -d">"