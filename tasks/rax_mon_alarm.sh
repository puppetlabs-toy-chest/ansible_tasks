...
if [ ! -z "${PT_notification_plan_id+x}" ];
  then params="${params} notification_plan_id=\"${PT_notification_plan_id}\""
fi
...
if [ ! -z "${PT_criteria+x}" ];
  then params="${params} criteria=\"${PT_criteria}\""
fi
...
ansible localhost -m rax_mon_alarm --args "${params}" --one-line | cut -f2- -d">"