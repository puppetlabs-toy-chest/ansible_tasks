...
if [ ! -z "${PT_provider_network_type+x}" ];
  then params="${params} provider_network_type=\"${PT_provider_network_type}\""
fi
...
if [ ! -z "${PT_provider_physical_network+x}" ];
  then params="${params} provider_physical_network=\"${PT_provider_physical_network}\""
fi
...
if [ ! -z "${PT_provider_segmentation_id+x}" ];
  then params="${params} provider_segmentation_id=\"${PT_provider_segmentation_id}\""
fi
...
if [ ! -z "${PT_shared+x}" ];
  then params="${params} shared=\"${PT_shared}\""
fi
...
ansible localhost -m os_network --args "${params}" --one-line | cut -f2- -d">"