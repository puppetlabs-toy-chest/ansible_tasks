...
if [ ! -z "${PT_route_target_both_auto_evpn+x}" ];
  then params="${params} route_target_both_auto_evpn=\"${PT_route_target_both_auto_evpn}\""
fi
...
ansible localhost -m nxos_vrf_af --args "${params}" --one-line | cut -f2- -d">"