...
if [ ! -z "${PT_localfs+x}" ];
  then params="${params} localfs=\"${PT_localfs}\""
fi
...
if [ ! -z "${PT_domain_function+x}" ];
  then params="${params} domain_function=\"${PT_domain_function}\""
fi
...
if [ ! -z "${PT_glusterfs+x}" ];
  then params="${params} glusterfs=\"${PT_glusterfs}\""
fi

if [ ! -z "${PT_iscsi+x}" ];
  then params="${params} iscsi=\"${PT_iscsi}\""
fi

if [ ! -z "${PT_posixfs+x}" ];
  then params="${params} posixfs=\"${PT_posixfs}\""
fi
...
if [ ! -z "${PT_destroy+x}" ];
  then params="${params} destroy=\"${PT_destroy}\""
fi

ansible localhost -m ovirt_storage_domains --args "${params}" --one-line | cut -f2- -d">"