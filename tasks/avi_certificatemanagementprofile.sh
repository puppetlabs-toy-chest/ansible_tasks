...
if [ ! -z "${PT_script_params+x}" ];
  then params="${params} script_params=\"${PT_script_params}\""
fi
...
if [ ! -z "${PT_script_path+x}" ];
  then params="${params} script_path=\"${PT_script_path}\""
fi
...
ansible localhost -m avi_certificatemanagementprofile --args "${params}" --one-line | cut -f2- -d">"