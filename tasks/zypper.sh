...
if [ ! -z "${PT_oldpackage+x}" ];
  then params="${params} oldpackage=\"${PT_oldpackage}\""
fi
...
if [ ! -z "${PT_disable_recommends+x}" ];
  then params="${params} disable_recommends=\"${PT_disable_recommends}\""
fi
...
ansible localhost -m zypper --args "${params}" --one-line | cut -f2- -d">"