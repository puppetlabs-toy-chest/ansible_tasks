...
if [ ! -z "${PT_agent_id+x}" ];
  then params="${params} agent_id=\"${PT_agent_id}\""
fi
...
if [ ! -z "${PT_named_ip_addresses+x}" ];
  then params="${params} named_ip_addresses=\"${PT_named_ip_addresses}\""
fi
...
ansible localhost -m rax_mon_entity --args "${params}" --one-line | cut -f2- -d">"