...
if [ ! -z "${PT_warning_state+x}" ];
  then params="${params} warning_state=\"${PT_warning_state}\""
fi
...
if [ ! -z "${PT_critical_state+x}" ];
  then params="${params} critical_state=\"${PT_critical_state}\""
fi
...
if [ ! -z "${PT_ok_state+x}" ];
  then params="${params} ok_state=\"${PT_ok_state}\""
fi
...
ansible localhost -m rax_mon_notification_plan --args "${params}" --one-line | cut -f2- -d">"