...
if [ ! -z "${PT_ironic_url+x}" ];
  then params="${params} ironic_url=\"${PT_ironic_url}\""
fi

if [ ! -z "${PT_maintenance+x}" ];
  then params="${params} maintenance=\"${PT_maintenance}\""
fi
...
if [ ! -z "${PT_power+x}" ];
  then params="${params} power=\"${PT_power}\""
fi
...
if [ ! -z "${PT_maintenance_reason+x}" ];
  then params="${params} maintenance_reason=\"${PT_maintenance_reason}\""
fi
...
if [ ! -z "${PT_instance_info+x}" ];
  then params="${params} instance_info=\"${PT_instance_info}\""
fi
...
ansible localhost -m os_ironic_node --args "${params}" --one-line | cut -f2- -d">"