...
params=''
if [ ! -z "${PT_vm_guest_ip+x}" ];
  then params="${params} vm_guest_ip=\"${PT_vm_guest_ip}\""
fi
...
if [ ! -z "${PT_private_port+x}" ];
  then params="${params} private_port=\"${PT_private_port}\""
fi
...
if [ ! -z "${PT_private_end_port+x}" ];
  then params="${params} private_end_port=\"${PT_private_end_port}\""
fi
...
if [ ! -z "${PT_open_firewall+x}" ];
  then params="${params} open_firewall=\"${PT_open_firewall}\""
fi

if [ ! -z "${PT_public_port+x}" ];
  then params="${params} public_port=\"${PT_public_port}\""
fi
...
if [ ! -z "${PT_public_end_port+x}" ];
  then params="${params} public_end_port=\"${PT_public_end_port}\""
fi
...
ansible localhost -m cs_portforward --args "${params}" --one-line | cut -f2- -d">"