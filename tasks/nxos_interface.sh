...
if [ ! -z "${PT_fabric_forwarding_anycast_gateway+x}" ];
  then params="${params} fabric_forwarding_anycast_gateway=\"${PT_fabric_forwarding_anycast_gateway}\""
fi
...
ansible localhost -m nxos_interface --args "${params}" --one-line | cut -f2- -d">"