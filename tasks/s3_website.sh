...
if [ ! -z "${PT_redirect_all_requests+x}" ];
  then params="${params} redirect_all_requests=\"${PT_redirect_all_requests}\""
fi

if [ ! -z "${PT_error_key+x}" ];
  then params="${params} error_key=\"${PT_error_key}\""
fi
...
ansible localhost -m s3_website --args "${params}" --one-line | cut -f2- -d">"