...
if [ ! -z "${PT_info+x}" ];
  then params="${params} info=\"${PT_info}\""
fi
...
if [ ! -z "${PT_hyp_props+x}" ];
  then params="${params} hyp_props=\"${PT_hyp_props}\""
fi
...
if [ ! -z "${PT_cc_vtypes+x}" ];
  then params="${params} cc_vtypes=\"${PT_cc_vtypes}\""
fi
...
if [ ! -z "${PT_cc_props+x}" ];
  then params="${params} cc_props=\"${PT_cc_props}\""
fi

ansible localhost -m avi_cloudproperties --args "${params}" --one-line | cut -f2- -d">"