...
if [ ! -z "${PT_notification_type+x}" ];
  then params="${params} notification_type=\"${PT_notification_type}\""
fi
...
ansible localhost -m rax_mon_notification --args "${params}" --one-line | cut -f2- -d">"