...
if [ ! -z "${PT_alertenable+x}" ];
  then params="${params} alertenable=\"${PT_alertenable}\""
fi
...
if [ ! -z "${PT_starttime+x}" ];
  then params="${params} starttime=\"${PT_starttime}\""
fi
...
ansible localhost -m logicmonitor --args "${params}" --one-line | cut -f2- -d">"