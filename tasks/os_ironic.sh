...
if [ ! -z "${PT_driver_info+x}" ];
  then params="${params} driver_info=\"${PT_driver_info}\""
fi
...
if [ ! -z "${PT_chassis_uuid+x}" ];
  then params="${params} chassis_uuid=\"${PT_chassis_uuid}\""
fi
...
if [ ! -z "${PT_skip_update_of_driver_password+x}" ];
  then params="${params} skip_update_of_driver_password=\"${PT_skip_update_of_driver_password}\""
fi

ansible localhost -m os_ironic --args "${params}" --one-line | cut -f2- -d">"