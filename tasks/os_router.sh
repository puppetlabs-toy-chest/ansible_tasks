...
if [ ! -z "${PT_enable_snat+x}" ];
  then params="${params} enable_snat=\"${PT_enable_snat}\""
fi
...
if [ ! -z "${PT_external_fixed_ips+x}" ];
  then params="${params} external_fixed_ips=\"${PT_external_fixed_ips}\""
fi
...
ansible localhost -m os_router --args "${params}" --one-line | cut -f2- -d">"