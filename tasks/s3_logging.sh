...
if [ ! -z "${PT_target_prefix+x}" ];
  then params="${params} target_prefix=\"${PT_target_prefix}\""
fi

if [ ! -z "${PT_target_bucket+x}" ];
  then params="${params} target_bucket=\"${PT_target_bucket}\""
fi
...
ansible localhost -m s3_logging --args "${params}" --one-line | cut -f2- -d">"