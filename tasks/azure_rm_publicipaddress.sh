...
if [ ! -z "${PT_domain_name_label+x}" ];
  then params="${params} domain_name_label=\"${PT_domain_name_label}\""
fi
...
if [ ! -z "${PT_allocation_method+x}" ];
  then params="${params} allocation_method=\"${PT_allocation_method}\""
fi
...
ansible localhost -m azure_rm_publicipaddress --args "${params}" --one-line | cut -f2- -d">"