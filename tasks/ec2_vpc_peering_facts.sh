...
if [ ! -z "${PT_peer_connection_ids+x}" ];
  then params="${params} peer_connection_ids=\"${PT_peer_connection_ids}\""
fi
...
ansible localhost -m ec2_vpc_peering_facts --args "${params}" --one-line | cut -f2- -d">"